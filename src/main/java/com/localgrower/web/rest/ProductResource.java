...
/**
 * REST controller for managing {@link com.localgrower.domain.Product}.
 */
@RestController
@RequestMapping("/api")
public class ProductResource {
    ...
    public ResponseEntity<ProductDTO> createProduct(@Valid @RequestBody ProductDTO productDTO) throws URISyntaxException {
        ...
        return ResponseEntity
            ...
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, result.getIdProduct().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /products/:idProduct} : Updates an existing product.
     *
     * @param idProduct the id of the productDTO to save.
    ...
    public ResponseEntity<ProductDTO> updateProduct(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, productDTO.getIdProduct().toString()))
            .body(result);
    }

    /**
     * {@code PATCH  /products/:idProduct} : Partial updates given fields of an existing product, field will ignore if it is null
     *
     * @param idProduct the id of the productDTO to save.
     * @param productDTO the productDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated productDTO,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/products/{idProduct}", consumes = "application/merge-patch+json")
    public ResponseEntity<ProductDTO> partialUpdateProduct(
        @PathVariable(value = "idProduct", required = false) final Long idProduct,
        @NotNull @RequestBody ProductDTO productDTO
    ) throws URISyntaxException {
        log.debug("REST request to partial update Product partially : {}, {}", idProduct, productDTO);
        if (productDTO.getIdProduct() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(idProduct, productDTO.getIdProduct())) {
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, productDTO.getIdProduct().toString())
        );
    }
