...
import com.localgrower.service.CategoryQueryService;
import com.localgrower.service.CategoryService;
...
/**
 * REST controller for managing {@link com.localgrower.domain.Category}.
 */
@RestController
@RequestMapping("/api")
public class CategoryResource {
     ...
     */
    @PostMapping("/adaugare/categorie")
    public ResponseEntity<CategoryDTO> createCategory(@Valid @RequestBody CategoryDTO categoryDTO) throws URISyntaxException {
        ...
        return ResponseEntity
            .created(new URI("/api/adaugare/categorie/" + result.getIdCategory()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, result.getIdCategory().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /categories/:idCategory} : Updates an existing category.
     *
     * @param idCategory the id of the categoryDTO to save.
    ...
    public ResponseEntity<CategoryDTO> updateCategory(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, categoryDTO.getIdCategory().toString()))
            .body(result);
    }

    /**
     * {@code PATCH  /categories/:idCategory} : Partial updates given fields of an existing category, field will ignore if it is null
     *
     * @param idCategory the id of the categoryDTO to save.
     * @param categoryDTO the categoryDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated categoryDTO,
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/categories/{idCategory}", consumes = "application/merge-patch+json")
    public ResponseEntity<CategoryDTO> partialUpdateCategory(
        @PathVariable(value = "idCategory", required = false) final Long idCategory,
        @NotNull @RequestBody CategoryDTO categoryDTO
    ) throws URISyntaxException {
        log.debug("REST request to partial update Category partially : {}, {}", idCategory, categoryDTO);
        if (categoryDTO.getIdCategory() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(idCategory, categoryDTO.getIdCategory())) {
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, categoryDTO.getIdCategory().toString())
        );
    }
